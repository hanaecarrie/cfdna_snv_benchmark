...
if __name__ == "__main__":
    ...
    mixtureids_train = ['CRC-123_310715-CW-T_CRC-123_121115-CW-T', 'CRC-986_100215-CW-T_CRC-986_300316-CW-T' ] # , 'BRA-412_240820-CW-T_BRA-412_060220-CW-T']
    mixtureid_test = 'CRC-1014_180816-CW-T_CRC-1014_090516-CW-T'

    classifiers = {'LogisticRegressionLasso': LogisticRegression(penalty='l1', solver='liblinear', random_state=0),
                    'LogisticRegressionRidge': LogisticRegression(penalty='l2', solver='liblinear', random_state=0),
                    'LogisticRegressionElasticnet': LogisticRegression(penalty='elasticnet', solver='saga', random_state=0),
                    'DecisionTree': DecisionTreeClassifier(random_state=0),
                    'MultiLayerPerceptron': MLPClassifier(random_state=0),
                   }
    ...
    for method in [m for m in config.methods if m != 'varnet' and m != 'abemus' and m != 'sinvict']:
        ...
        #classifiername = 'LogisticRegressionLasso'
        #classifiermodel = LogisticRegression(penalty='l1', solver='liblinear', random_state=0)

        classifiername = 'DecisionTree'
        classifiermodel = DecisionTreeClassifier(random_state=0)
        ...
        if not os.path.exists('data/featureanalysis/Xtrain_'+method+'_without'+patientid+'.npy'): #or not not os.path.exists('data/featureanalysis/ytrue_'+method+'_'+patientid+'.csv'):
            ...
            forbiden_features = ['AF', 'OLD_VARIANT', 'TYPE', "cosmid_id", 'AN', "NS", 'CIGAR', 'DP', 'AF', 'AO', 'VD', 'N_refDepth', 'N_altDepth', 'relcov', 'END_POS_REF', 'REF_MFVdVs', 'ALT_MFVdVs', 'Sample_Name', 'cov', 'tf', method, method+'_score', method+'_vaf', 'type', 'alt', 'chrom', 'pos', 'ref', method+'_totcov', method+'_altcov']
            features_selected = [c for c in X.columns if c not in forbiden_features]
            if method == 'mutect2':
            ...
            np.save('data/featureanalysis/Xtrain_'+method+'_without'+patientid+'.npy', X)
            np.save('data/featureanalysis/ytrain_'+method+'_without'+patientid+'.npy', y)

        else:  # if os.path.exists('data/featureanalysis/Xtrain_'+method+'_without'+patientid+'.npy'):
            X = np.load('data/featureanalysis/Xtrain_'+method+'_without'+patientid+'.npy')
            y = np.load('data/featureanalysis/ytrain_'+method+'_without'+patientid+'.npy')
            cfdnaspecific_modeRFC = classifiermodel
            cfdnaspecific_modeRFC.fit(X, y)
        ...
        # feature_importances.to_csv('data/featureanalysis/barplot_featureimportance_'+classifiername+'_'+method+'_cfDNAtuned_leftoutpatient'+patientid+'_150x_RF.csv')
        # plt.savefig('data/featureanalysis/featureimportance_'+classifiername+'_'+method+'_trainpatients_150x.svg')
        # plt.savefig('data/featureanalysis/featureimportance_'+classifiername+'_'+method+'_trainpatients_150x.png')
        # #plt.show()
        ...
        if not os.path.exists('data/featureanalysis/ytrue_'+method+'_'+patientid+'.csv'):
            ...
            # edited
            features_selected = callmethod_snv_allchrom.columns
            forbiden_features = ['AF', 'OLD_VARIANT', 'TYPE', "cosmid_id", 'AN', "NS", 'CIGAR', 'DP', 'AF', 'AO', 'VD', 'N_refDepth', 'N_altDepth', 'relcov', 'END_POS_REF', 'REF_MFVdVs', 'ALT_MFVdVs', 'Sample_Name', 'cov', 'tf', 'LOF', method, method+'_score', method+'_vaf', 'type', 'alt', 'chrom', 'pos', 'ref', method+'_totcov', method+'_altcov']
            features_selected = [c for c in features_selected if c not in forbiden_features]
            if method == 'mutect2':
                features_selected = [f for f in features_selected if f not in ['N_ART_LOD', 'POP_AF', 'P_GERMLINE']]
            X = callmethod_snv_allchrom[list(features_selected)].fillna(0)
            ...
            for feature in features_selected:
                ...
                if feature not in forbiden_features:
                    ...
                    if test == 1 and feature != 'truth' and feature not in forbiden_features:
                        ...
                        X = X.drop(feature, axis=1, inplace=True)
                        print(feature, 'not included')
                else:
                    X = X.drop(feature, axis=1, inplace=True)
                    print(feature, 'not included because forbidden')
            ...
            # save X test
            #X.to_csv('data/featureanalysis/Xtest_'+method+'_'+patientid+'.csv')
            ...
            plt.xlim([0.01, 1])
            plt.title(method)
            plt.savefig('data/featureanalysis/PRcuvres_naive_finetuned_'+classifiername+'_'+method+'_'+patientid+'.png', bbox_inches='tight')
            plt.savefig('data/featureanalysis/PRcuvres_naive_finetuned_'+classifiername+'_'+method+'_'+patientid+'.svg', bbox_inches='tight')
            metric = 'maxrecallatleast0_03precision'
            threshold = float(metric.split('_')[1].split('precision')[0])/100
            print(threshold)
            p = np.where(precision >= threshold, 1, 0)
            ...
            dfrescfdna.to_csv('data/featureanalysis/PRAUPRC_featureimportance_'+classifiername+'_'+method+'_'+patientid+'_150x_cfdnaspecific.csv')
            #print(metricscorespecific - metricscorenaive)

        else:
            X = pd.read_csv('data/featureanalysis/Xtest_'+method+'_'+patientid+'.csv', index_col=0)
            yscores = cfdnaspecific_modeRFC.predict_proba(X)[:,1]  # take predictions for class 1
            # y_true
            ytrue = pd.read_csv('data/featureanalysis/ytrue_'+method+'_'+patientid+'.csv', index_col=0)
            precision, recall, thresholds = precision_recall_curve(ytrue, yscores)
            print(average_precision_score(ytrue, yscores))
            metricscorespecific = average_precision_score(ytrue, yscores)
            figax = plt.subplots(figsize=(12,12))
