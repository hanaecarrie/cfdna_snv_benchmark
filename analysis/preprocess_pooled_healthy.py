...
if args.snp_database == 'dbsnp':
    ...
    reads2remove, log_df = list_reads_to_remove("../data/healthy_chr22_merged-ready.bam",
                                                dbsnp_df, patient_snps_df, verbose=-1)
    # save list of reads to remove and log dataframe
    log_df.to_csv('../data/prepare_pooled_healthy/log_'+patient_date.split('_')[0]+'_dbsnp.csv', index=False)
    ...
    # save file
    bamsurgeon_snv_pd.to_csv('../data/prepare_pooled_healthy/varfile_snv_'+patient_date.split('_')[0]+'_dbsnp.bed',
                             sep='\t', header=False, index=False)
    bamsurgeon_indel_pd.to_csv('../data/prepare_pooled_healthy/varfile_indel_'+patient_date.split('_')[0]+'_dbsnp.bed',
                               sep='\t', header=False, index=False)
    ...
    for genomad_df_chunk in genomad_df_iterator:
        ...
        if ci in args.chunk_list:
            ...
            # save list of reads to remove and log dataframe
            log_df.to_csv('../data/prepare_pooled_healthy/log_'+patient_date.split('_')[0]+'_genomad_'+str(ci)+'.csv', index=False)
            ...
            bamsurgeon_snv_pd, bamsurgeon_indel_pd = prepare_bamsurgeon_inputs(patient_snps_df, log_df, max_vaf=0.1)
            # save file
            bamsurgeon_snv_pd.to_csv('../data/prepare_pooled_healthy/varfile_snv_'+patient_date.split('_')[0]+'genomad_'+str(ci)+'.bed',
                                     sep='\t', header=False, index=False)
            bamsurgeon_indel_pd.to_csv('../data/prepare_pooled_healthy/varfile_indel_'+patient_date.split('_')[0]+'genomad_'+str(ci)+'.bed',
                                       sep='\t', header=False, index=False)
