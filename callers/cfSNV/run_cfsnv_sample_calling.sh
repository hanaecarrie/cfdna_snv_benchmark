...
exec 1>$outdirplasma/log_calling.out 2>&1
...
Rscript run_parameter.R --config_file $config_file --plasmaid $plasmaid ;
tail -n 13 $outdirplasma/log_calling.out > $outdirplasma/parameter.txt ;
#fi

# apply cfSNV mutation calling function per batch
if [ ! -d $outdirplasma/results ] ; then mkdir $outdirplasma/results ; fi
for targetbed in $extdata/exome_bed/exome_hg19_merge_chr${chr}_*.bed ; do 
	echo $targetbed 
...
hmscfsnv=$(printf '%02dh:%02dm:%02ds\n' $((timecfsnv/3600)) $((timecfsnv%3600/60)) $((timecfsnv%60)))
        echo "Elapsed Time cfSNV calling on ${sampleid} for chr${chr}: ${hmscfsnv}"
        echo "Elapsed Time cfSNV calling on ${sampleid} for chr${chr}: ${hmscfsnv}" >> $outdir/logtime_calling.out
