...
exec 1>$outdir/log_calling.out 2>&1
# Everything below will go to the file 'log.out':

# Start logging the RAM usage and CPU usage
bash ${repopath}/ABEMUS/log_mem_cpu.sh $outdir/log_mem_cpu_calling.out  & export logpid=$!

#echo 'Clean tmp folder'
#echo $tmpdir
#rm -r $tmpdir

touch $outdir/logtime_calling.out
...
for plasma in ${dilutionseriesfolder}/*/*[Tx].bam ; do
        echo "plasma ${plasma}" ;
        bash ${repopath}/cfSNV/run_cfsnv_sample_calling.sh -c $config_file -p $plasma 
done
