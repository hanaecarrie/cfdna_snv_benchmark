...
def get_featuretable(calldir, methods, save=False, filter='PASS'):
    ...
    for method in methods:
        ...
        if method in ['freebayes', 'mutect2', 'strelka2', 'vardict', 'varscan']:
            ...
            if not os.path.exists(calltablemethod_path):
                print('calls for caller {} do not exist. path {} not found.'.format(method, calltablemethod_path))
                callmethod = pd.DataFrame(columns=['chrom', 'pos', 'ref', 'alt', 'type', 'XXX'])
            else:
                callmethod = pd.read_csv(calltablemethod_path)
                callmethod.drop(['PatientID', 'CaseID', 'GermlineID'], inplace=True)
                callmethod.rename(columns={'chr': 'chrom', 'dbsnp': 'type'})
            ...
            for lev in range(1, 7):
                ...
                if not os.path.exists(calltablemethod_path):
                    ...
                    callmethod_sinvict.drop('samplename', axis=1, inplace=True)
                    ...
                    callmethod_sinvict['+strand'] = callmethod_sinvict['+strand'].str[2:]
                    callmethod_sinvict['-strand'] = callmethod_sinvict['+strand'].str[2:]
            ...
            callmethod['levels'] = callmethod[[method+'_level'+str(le) for le in range(1, 7)]].sum(axis=1)
        else:
